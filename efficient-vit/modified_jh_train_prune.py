...
def process_video_frames(
    video_path: str, config: dict, model: nn.Module = None
    ...
    for frame_path in glob.glob(os.path.join(video_path, "*")):
        try:
            ...
            if image is not None:
                ...
                image = frame_transform(image).numpy()
...
def setup_data_loaders(
    ...
    model = EfficientViT(
        config=config.__dict__, channels=1280, selected_efficient_net=0
    ).to(config.device)
    model.eval()
    ...
    for path in tqdm(paths, desc="Processing frames"):
        try:
            frames = process_video_frames(path, config.__dict__, model)
            if TRAINING_DIR in path:
                train_dataset.extend(frames)
            else:
                validation_dataset.extend(frames)
    ...
    train_loader = DataLoader(
        DeepFakesDataset(
            np.array([row[0] for row in train_dataset]),
            np.array([row[1] for row in train_dataset]),
            config.image_size,
        ),
    ...
    val_loader = DataLoader(
        DeepFakesDataset(
            np.array([row[0] for row in validation_dataset]),
            np.array([row[1] for row in validation_dataset]),
            config.image_size,
            mode="validation",
        ),
        batch_size=config.batch_size,
