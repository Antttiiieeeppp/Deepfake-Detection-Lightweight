...
from jh_efficientprune_vit import EfficientViT
import uuid
from torch.utils.data import DataLoader, TensorDataset, Dataset
...
from sklearn.linear_model import Lasso #pruning을 위한 lasso 회귀 계산
...
RESULT_PATH = f"/home/work/Antttiiieeeppp/jaehee/result/{current_time}"
...
def read_frames(video_path, train_dataset, validation_dataset):
    ...
    if TRAINING_DIR in video_path:
        ...
        if "Original" in video_path:
            ...
            # video_key = video_folder_name #sample dataset
...
if __name__ == "__main__":
    ...
    #                     help="Rank ratio for SVD pruning (0~1).") #추가 SVD
    parser.add_argument('--prune_ratio', type=float, default=0.5, 
                        help="Prune ratio for Structured Pruning (0~1).")
    ...
    # # SVD 프루닝 호출
    # print("Pruning EfficientNet layers with SVD...")
    # model.svd_prune_efficientnet(layer_indices=[-3, -2, -1], rank_ratio=opt.svd_rank_ratio)
    # Structured Pruning 호출
    print("Pruning EfficientNet layers with Structured Pruning...")
    model.structured_prune_efficientnet(layer_indices=[-3, -2, -1], prune_ratio=opt.prune_ratio)
    ...
    print("Model Parameters After Pruning:", get_n_params(model)) #pruning후 모델 파라미터 계산 업데이트
    ...
    # for t in range(starting_epoch, opt.num_epochs):
        ...
        if val_f1_score == max(val_f1_score_list):
            torch.save(model.state_dict(), os.path.join(RESULT_PATH,  "pruning_efficientnetB" + str(opt.efficient_net)))
